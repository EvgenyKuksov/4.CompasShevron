...
function AddLine1(LSS: ILineDimensions; X1, Y1, X2, Y2, X3, Y3: extended): ILineDimension;
var
  LS2: ILineDimension;
...
LineSeg2[19] := AddLine1(Skk, x0 + da2 / 2+30+Lcm2, y0 +dv2/2, x0 + da2 / 2+30, y0 + dv2 / 2, x0 + da2 / 2+30+Lcm2/2, y0 -dcm2/2);
...
rounghs.Add;  //шероховатость
rounghs.Rough[1].X0:=x0+da2/2+30;
rounghs.Rough[1].Y0:=y0+Dv2/4;
...
Toler:=Tolers.Add;
	...
	begin
		...
		branchs := toler As IBranchs;
 		if ( branchs <> nil ) then
		begin
			branchs.X0 := x0+da2/2+lcm2/2+30;   	// Задать точку привязки
			branchs.Y0 := y0-dv2/6;
			branchs.AddBranchByPoint( 1, x0+da2/2+lcm2/2+30, y0+dv2/2);	// Добавить ответвление
		end;
...
begin
		// Получить интерфейс таблицы с текстом допуска формы
		  ...
		  begin
			...
			TolerPar.BasePointPos := ksTPUpCenter;
		end;
		...
		Toler.Set_BranchPos( 0, ksTPUpCenter );
  toler.Update();
	end;
